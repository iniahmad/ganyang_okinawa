...
module encoder_fixed_point_sequential #(
    ...
    reg signed [BITSIZE-1:0] mult_result_reg [0:N_input-1][0:M_output-1];    // Hasil perkalian
    wire signed [BITSIZE-1:0] tree_sum_result [0:M_output-1];             // Hasil adder tree untuk setiap output
    reg signed [BITSIZE-1:0] tree_sum_result_reg [0:M_output-1];             // Hasil adder tree untuk setiap output
    wire signed [BITSIZE-1:0] final_result [0:M_output-1];                // Hasil akhir setelah bias
    reg signed [BITSIZE-1:0] final_result_reg [0:M_output-1];                // Hasil akhir setelah bias
    ...
    generate
        for (j = 0; j < M_output; j = j + 1) begin : gen_adder_tree
            ...
            always @(posedge clk or posedge reset) begin
                if (reset) begin
                    tree_sum_result_reg[j] <= 0;
                end else begin
                    tree_sum_result_reg[j] <= tree_sum_result[j];
                end
            end
        end
    endgenerate
        ...
        for (j = 0; j < M_output; j = j + 1) begin : gen_bias
            ...
            always @(posedge clk or posedge reset) begin
                if (reset) begin
                    ...
                    final_result_reg[j] <= final_result[j];
                end
            end
